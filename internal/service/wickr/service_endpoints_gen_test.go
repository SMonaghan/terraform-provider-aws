...
const (
	packageName = "wickr"
	awsEnvVar   = "AWS_ENDPOINT_URL_WICKR"
...
func defaultEndpoint(ctx context.Context, region string) (url.URL, error) {
	r := wickr.NewDefaultEndpointResolverV2()

	ep, err := r.ResolveEndpoint(ctx, wickr.EndpointParameters{
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.WickrClient(ctx)

	var result apiCallParams

	input := wickr.ListNetworksInput{}
	_, err := client.ListNetworks(ctx, &input,
		func(opts *wickr.Options) {
...
func expectDefaultEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Wickr default endpoint: %s", err)
	}
...
func expectDefaultFIPSEndpoint(ctx context.Context, t *testing.T, region string) caseExpectations {
	...
	if err != nil {
		t.Fatalf("resolving Wickr FIPS endpoint: %s", err)
	}
	...
	if dnsErr, ok := errors.AsType[*net.DNSError](err); ok && (dnsErr.IsNotFound || dnsErr.IsTimeout) {
		...
		t.Fatalf("looking up Wickr endpoint %q: %s", hostname, err)
	}
